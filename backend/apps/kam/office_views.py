...
def _serialize_kam_office_account(ent: Enterprise) -> dict:
    """Helper to serialize an enterprise for KAM Office listing."""
...
def _filter_kam_office_accounts_qs(params):
    """Helper to filter KAM Office accounts queryset based on request parameters."""
    segment = params.get('segment', 'ALL')
    assignment = params.get('assignment', 'ALL')
    kam_id = params.get('kam_id', None)
