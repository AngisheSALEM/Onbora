...
from accounts.permissions import IsSalespersonOrAdmin, IsAdmin, IsSupervisorOrAdmin, IsSupervisor, IsSalesperson
...
class PlaqueListCreateView(APIView):
    ...
    POST: Crée une nouvelle plaque de prospection territoriale (réservé aux superviseurs et administrateurs).
    """
    ...
    def get(self, request):
        try:
            ...
            plaques_dto = use_case.execute(request=request)
            if request.query_params.get('assigned_only') == 'true' and request.user.is_authenticated:
                plaques_dto = [p for p in plaques_dto if request.user.id in p.assigned_salespersons]
            serializer = PlaqueSerializer(plaques_dto, many=True, context={'request': request})
            ...
            if request.query_params.get('assigned_only') == 'true' and request.user.is_authenticated:
                plaques = plaques.filter(assigned_salespersons=request.user)
            serializer = PlaqueSerializer(plaques, many=True, context={'request': request})
            return Response(serializer.data, status=status.HTTP_200_OK)

    def post(self, request):
        if not (request.user.role in ['SUPERVISOR', 'ADMIN'] or request.user.is_superuser):
            return Response({"detail": "Action non autorisée. Réservé aux superviseurs et administrateurs."}, status=status.HTTP_403_FORBIDDEN)
...
class PlaqueDetailView(APIView):
    ...
    def get(self, request, pk):
        try:
            ...
            serializer = PlaqueDetailSerializer(plaque_obj, context={'request': request})
            return Response(serializer.data, status=status.HTTP_200_OK)
...
def _persist_drawn_plaque(data: dict) -> tuple:
    """Parses geometry, creates or updates Plaque, and returns (plaque, salespersons)."""
    code = (data.get('code') or '').strip().upper()
    name = (data.get('name') or '').strip()
    city = (data.get('city') or 'Kinshasa').strip()
    plaque_id = data.get('plaque_id')
    salesperson_ids = data.get('salesperson_ids', [])

    try:
        lat_f = float(data.get('latitude', -4.3033))
        lng_f = float(data.get('longitude', 15.3083))
        rad_f = float(data.get('radius_km', 5.0))
    except (ValueError, TypeError):
        lat_f, lng_f, rad_f = -4.3033, 15.3083, 5.0

    if plaque_id:
        plaque = Plaque.objects.get(pk=plaque_id)
    else:
        plaque = Plaque.objects.filter(code=code).first() or Plaque(code=code)
    ...
    if data.get('boundary_geojson'):
        plaque.boundary_geojson = data.get('boundary_geojson')
    if data.get('kml_data'):
        plaque.kml_data = data.get('kml_data')
...
class PlaqueDrawAndSaveView(APIView):
    ...
    def post(self, request):
        try:
            ...
            try:
                plaque, salespersons = _persist_drawn_plaque(request.data)
            except Plaque.DoesNotExist:
                return Response({"detail": "Plaque introuvable."}, status=status.HTTP_404_NOT_FOUND)

            _notify_plaque_drawn_salespersons(plaque, salespersons)
            ...
            try:
                log_demo_event(
                    ...
                    user=request.user if request.user.is_authenticated else None,
                    metadata={"plaque_id": plaque.id, "salesperson_ids": list(request.data.get('salesperson_ids', []))}
                )
            except Exception:
                pass
...
def _get_dashboard_plaques_summary():
    """Calculates grouped plaque counters and returns serialized plaques."""
    total_ent_map = dict(Enterprise.objects.filter(plaque_rel__isnull=False).values_list('plaque_rel_id').annotate(c=models.Count('id')))
    ready_ent_map = dict(Enterprise.objects.filter(plaque_rel__isnull=False, is_ready_for_conversion=True).values_list('plaque_rel_id').annotate(c=models.Count('id')))
    plaque_ctx = {
        'total_enterprises_map': total_ent_map,
        'ready_count_map': ready_ent_map,
    }
    plaques = ListPlaquesUseCase().execute()
    return PlaqueSerializer(plaques, many=True, context=plaque_ctx).data


def _get_dashboard_salespersons_summary():
    """Precomputes batch metrics for salespersons and serializes them."""
    from accounts.models import User
    reports_map = dict(VisitReport.objects.filter(preparation__salesperson__isnull=False).values_list('preparation__salesperson_id').annotate(c=models.Count('id')))
    submissions_map = dict(VisitFormSubmission.objects.filter(salesperson__isnull=False).values_list('salesperson_id').annotate(c=models.Count('id')))
    conv_map = dict(Enterprise.objects.filter(conversion_status='CONVERTED', converted_by_user__isnull=False).values_list('converted_by_user_id').annotate(c=models.Count('id')))
    amount_map = {row[0]: float(row[1] or 0.0) for row in Enterprise.objects.filter(conversion_status='CONVERTED', converted_by_user__isnull=False).values_list('converted_by_user_id').annotate(t=models.Sum('converted_amount'))}
    points_map = {row[0]: int(row[1] or 0) for row in SalesIncentivePoint.objects.filter(salesperson__isnull=False).values_list('salesperson_id').annotate(t=models.Sum('points'))}
    ...
    return salespersons, salespersons_data


def _get_dashboard_enterprises_summary(request):
    """Retrieves enterprise counts and serialized cockpit records."""
    entity_filter = request.query_params.get('assigned_entity', 'BACK_OFFICE')
    base_counts_qs = Enterprise.objects.all()
    if entity_filter and entity_filter != 'ALL':
        base_counts_qs = base_counts_qs.filter(assigned_entity=entity_filter)

    ent_counts = base_counts_qs.aggregate(
        total=models.Count('id'),
        ready=models.Count('id', filter=models.Q(is_ready_for_conversion=True))
    )
    ...
    ).select_related('assigned_kam', 'assigned_salesperson', 'plaque_rel', 'last_visited_by').order_by('-created_at')

    if entity_filter and entity_filter != 'ALL':
        enterprises_qs = enterprises_qs.filter(
            models.Q(assigned_entity=entity_filter) | models.Q(is_visited=True) | models.Q(assigned_salesperson__isnull=False)
        )
    ...
    try:
        ...
        limit = 1000
    enterprises_data = EnterpriseCockpitSerializer(enterprises_qs[:limit], many=True).data
    return total_enterprises, ready_count, enterprises_data


def _get_dashboard_recent_feeds():
    """Builds recent activity feeds for reports and form submissions."""
    reports = VisitReport.objects.select_related('preparation__enterprise', 'preparation__salesperson', 'preparation__enterprise__plaque_rel').order_by('-created_at')[:100]
    reports_feed = []
    for r in reports:
        ...
        if hasattr(r, 'preparation') and r.preparation:
            if r.preparation.salesperson:
                salesperson_id = r.preparation.salesperson.id
            ...
            if r.preparation.enterprise:
                ent_id = r.preparation.enterprise.id
                ent_name = r.preparation.enterprise.name
                if r.preparation.enterprise.plaque_rel:
                    plaque_code = r.preparation.enterprise.plaque_rel.code
                else:
                    plaque_code = str(r.preparation.enterprise.plaque or "")
    ...
    submissions = VisitFormSubmission.objects.select_related('enterprise', 'salesperson', 'enterprise__plaque_rel').order_by('-created_at')[:100]
    submissions_feed = []
    for s in submissions:
        sp_name = "Commercial Terrain"
        sp_id = None
        if s.salesperson:
            sp_id = s.salesperson.id
            sp_name = f"{s.salesperson.first_name} {s.salesperson.last_name}".strip() or s.salesperson.username
        plaque_c = s.enterprise.plaque_rel.code if (s.enterprise and s.enterprise.plaque_rel) else (str(s.enterprise.plaque or "") if s.enterprise else "")
        submissions_feed.append({
            "id": s.id,
            "salesperson_id": sp_id,
            "salesperson_name": sp_name,
            "enterprise_id": s.enterprise_id,
            "enterprise_name": s.enterprise.name if s.enterprise else "Entreprise",
            "enterprise_sector": s.enterprise.sector if s.enterprise else "",
            "enterprise_commune": s.enterprise.commune if s.enterprise else "",
            "plaque_code": plaque_c,
            "target_offer_name": s.target_offer_name,
            "answers": s.answers,
            "ai_summary": s.ai_summary,
            "qualification_score": s.qualification_score,
            "detected_needs": s.detected_needs,
            "objections_noted": s.objections_noted,
            "next_action": s.next_action,
            "status": s.status,
            "created_at": s.created_at.isoformat() if hasattr(s.created_at, 'isoformat') else str(s.created_at),
        })

    return reports_feed, submissions_feed
...
class SupervisorDashboardView(APIView):
    ...
    def get(self, request):
        plaques_data = _get_dashboard_plaques_summary()
        salespersons, salespersons_data = _get_dashboard_salespersons_summary()
        total_enterprises, ready_count, enterprises_data = _get_dashboard_enterprises_summary(request)
        reports_feed, submissions_feed = _get_dashboard_recent_feeds()
    ...
    def post(self, request):
        ...
        try:
            ...
            return Response({
                ...
                "email_j1": result_dto.email_j1,
                "email_j4": result_dto.email_j4,
                "bant_score": result_dto.bant_score,
                "coi_metrics": result_dto.coi_metrics,
                "tiered_packages": result_dto.tiered_packages,
                "technical_handover_specs": result_dto.technical_handover_specs,
                "original_ai_output": {
                    "bant": result_dto.bant_score,
                    "coi": result_dto.coi_metrics,
                    "packages": result_dto.tiered_packages,
                    "email_j1": result_dto.email_j1,
                    "email_j4": result_dto.email_j4,
                    "technical_handover_specs": result_dto.technical_handover_specs,
                },
...
class VisitPreparationCreateView(APIView):
    ...
    def get(self, request):
        user = request.user
        completed_only = request.query_params.get('completed_only', '').lower() in ['true', '1', 'yes']
        history = request.query_params.get('history', '').lower() in ['true', '1', 'yes']
        ...
        # Par défaut, l'historique des visites terrain ne doit afficher que les visites réellement effectuées (avec rapport)
        if not include_all or completed_only or history:
            preps = preps.filter(report__isnull=False)
...
class VisitReportCreateView(APIView):
    ...
    def get(self, request):
        ...
        if user.is_authenticated and user.role == 'SALESPERSON':
            reports = reports.filter(preparation__salesperson=user)
        elif sp_id:
            reports = reports.filter(preparation__salesperson_id=sp_id)
        ...
        for r in reports:
            sp_id_val = None
            sp_name_val = "Commercial Terrain"
            ent_id_val = None
            ent_name_val = "Entreprise"
            plaque_code_val = ""
            if hasattr(r, 'preparation') and r.preparation:
                if r.preparation.salesperson:
                    sp_id_val = r.preparation.salesperson.id
                    sp_name_val = f"{r.preparation.salesperson.first_name} {r.preparation.salesperson.last_name}".strip() or r.preparation.salesperson.username
                if r.preparation.enterprise:
                    ent_id_val = r.preparation.enterprise.id
                    ent_name_val = r.preparation.enterprise.name
                    if r.preparation.enterprise.plaque_rel:
                        plaque_code_val = r.preparation.enterprise.plaque_rel.code
                    else:
                        plaque_code_val = str(r.preparation.enterprise.plaque or "")

            ai_out = r.original_ai_output if isinstance(r.original_ai_output, dict) else {}
            results.append({
                "id": r.id,
                "preparation_id": r.preparation.id if hasattr(r, 'preparation') and r.preparation else None,
                "salesperson_id": sp_id_val,
                "salesperson_name": sp_name_val,
                "enterprise_id": ent_id_val,
                "enterprise_name": ent_name_val,
                "plaque_code": plaque_code_val,
                "executive_summary": r.executive_summary,
                "confirmed_needs": r.confirmed_needs,
                "objections_raised": r.objections_raised,
                "actions_todo": r.actions_todo,
                "follow_up_email_draft": r.follow_up_email_draft,
                "bant_score": ai_out.get('bant'),
                "coi_metrics": ai_out.get('coi'),
                "tiered_packages": ai_out.get('packages', []),
                "original_ai_output": ai_out,
                "processing_time_seconds": ai_out.get('processing_time_seconds'),
                "ai_feedback_rating": r.ai_feedback_rating,
                "ai_feedback_comments": r.ai_feedback_comments,
                "created_at": r.created_at.isoformat() if hasattr(r.created_at, 'isoformat') else str(r.created_at),
            })
...
class VisitReportTransmitView(APIView):
    ...
    def post(self, request, pk):
        try:
            ...
            return Response({
                "detail": "Rapport transmis au Back-Office avec succès.",
...
def _process_referrals(referrals_data: list, report, enterprise, user) -> int:
    """Creates ReferralLead entries and awards incentive points up to limit."""
    credited = 0
...
def _process_trade_audits(trade_audits_data: list, report, enterprise, user) -> int:
    """Creates TradeAudit entries and awards incentive points up to limit."""
    credited = 0
...
class FieldIntelligenceReportCreateListView(APIView):
    ...
    def post(self, request):
        ...
        visit_report = VisitReport.objects.filter(pk=visit_report_id).first() if visit_report_id else None
        ...
        total_points += _process_nearby_leads(nearby_leads_data, report, enterprise, request.user)

        referrals_data = data.get('referrals', [])
        total_points += _process_referrals(referrals_data, report, enterprise, request.user)

        trade_audits_data = data.get('trade_audits', [])
        total_points += _process_trade_audits(trade_audits_data, report, enterprise, request.user)
...
class VisitFormSubmissionListView(APIView):
    ...
    def get(self, request):
        ...
        queryset = VisitFormSubmission.objects.select_related('enterprise', 'salesperson', 'enterprise__plaque_rel').order_by('-created_at')
        if enterprise_id:
...
def _filter_enterprises_for_list(request):
    ...
    plaque_id = request.query_params.get('plaque_id')
    plaque_code = request.query_params.get('plaque') or request.query_params.get('plaque_code')
    assigned_only = request.query_params.get('assigned_only', '').lower() in ['true', '1', 'yes']
    assigned_to_me = request.query_params.get('assigned_to_me', '').lower() in ['true', '1', 'yes']
    search = request.query_params.get('search', '').strip()
    ...
    if plaque_id:
        qs = qs.filter(plaque_rel_id=plaque_id)
    if plaque_code and plaque_code != 'Toutes':
        qs = qs.filter(models.Q(plaque_rel__code__iexact=plaque_code) | models.Q(plaque__iexact=plaque_code))
    if search:
        qs = qs.filter(
            models.Q(name__icontains=search) |
    ...
    if request.user.is_authenticated and request.user.role == 'SALESPERSON':
        user_plaques = request.user.assigned_plaques.all()
        qs = qs.filter(assigned_entity='BACK_OFFICE')

        if assigned_only or assigned_to_me:
            qs = qs.filter(assigned_salesperson=request.user)
        elif not plaque_id and not (plaque_code and plaque_code != 'Toutes'):
            qs = qs.filter(
                models.Q(assigned_salesperson=request.user) |
                (models.Q(plaque_rel__in=user_plaques) & (models.Q(assigned_salesperson__isnull=True) | models.Q(assigned_salesperson=request.user)))
            )
