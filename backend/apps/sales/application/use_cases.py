...
class GenerateVisitReportWithAIUseCase(BaseUseCase[Tuple[int, Optional[str], Any], PostVisitReportResultDTO]):
    ...
    def execute(self, params: Tuple[int, Optional[str], Any]) -> PostVisitReportResultDTO:
        ...
        ai_duration_sec = round(time.perf_counter() - start_ai_time, 2)

        # 2. Fusionner les offres cochées en direct par le commercial avec les besoins confirmés (si échange exploitable)
        confirmed_needs = list(qual_res.detected_needs)
        if qual_res.bant.status != 'INSUFFICIENT_DATA' and live_session and isinstance(live_session.live_proposition, dict):
        ...
        ai_payload = {
            "processing_time_seconds": ai_duration_sec,
            "bant": {
    ...
    def execute(self, user: Any) -> SalespersonActivityDTO:
        if not user or not user.is_authenticated:
            return SalespersonActivityDTO(
                ...
                recent_reports=[
                    {"id": 1, "enterprise_name": "Pharmacie du Centre", "date": "Hier", "summary": "Intérêt confirmé.", "status": "TRANSMIS_BACKOFFICE"}
                ],
...
class CreateVisitReportUseCase(BaseUseCase[Tuple[int, str, Optional[str], Any], VisitReport]):
    """
    Génère le compte-rendu de visite du commercial terrain directement via le Core AI (Gemini).
    Remplace les anciens gabarits statiques par une vraie analyse d'intelligence commerciale.
    """
    def execute(self, params: Tuple[int, str, Optional[str], Any]) -> VisitReport:
        ...
        # Délégation directe au moteur Core AI d'analyse de visite
        ai_use_case = GenerateVisitReportWithAIUseCase()
        result_dto = ai_use_case.execute((prep_id, raw_transcript, user))

        report = VisitReport.objects.get(pk=result_dto.report_id)
        if audio_file_path:
            report.audio_file_path = audio_file_path
            report.save(update_fields=['audio_file_path'])
...
class SubmitVisitFormUseCase(BaseUseCase[Tuple[Dict[str, Any], Any], Dict[str, Any]]):
    def execute(self, params: Tuple[Dict[str, Any], Any]) -> Dict[str, Any]:
        ...
        # 3.1 Marquage de l'entreprise comme visitée
        from django.utils import timezone
        enterprise.is_visited = True
        enterprise.last_visited_at = timezone.now()
        if user and user.is_authenticated:
            enterprise.last_visited_by = user
        enterprise.save(update_fields=['is_visited', 'last_visited_at', 'last_visited_by'])

        # 4. Création ou mise à jour de la VisitPreparation et VisitReport associée pour la continuité
        prep, _ = VisitPreparation.objects.get_or_create(
            enterprise=enterprise,
            defaults={
                'salesperson': user if (user and user.is_authenticated) else None,
        ...
        if user and user.is_authenticated and prep.salesperson != user:
            prep.salesperson = user
            prep.save(update_fields=['salesperson'])
        ...
        if not created:
            report.executive_summary = ai_summary
            report.confirmed_needs = detected_needs
            if objections_noted:
                report.objections_raised = [objections_noted]
            report.save(update_fields=['executive_summary', 'confirmed_needs', 'objections_raised'])
