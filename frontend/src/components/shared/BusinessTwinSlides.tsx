...
export default function BusinessTwinSlides({
      ...
      case 0:
        return (
          <div className="flex flex-col items-center justify-center text-center h-full gap-4 animate-fade-in p-6">
            ...
            <h2 className="text-xl font-extrabold tracking-tight text-zinc-50 uppercase">
              Transformation MSP
            </h2>
            <div className="h-0.5 w-12 orange-gradient-bg" />
            <p className="text-sm text-zinc-300 max-w-xs font-medium">
        ...
        return (
          <div className="flex flex-col h-full justify-between animate-fade-in p-5">
            <div>
              ...
              <h3 className="text-sm font-bold text-zinc-50 mt-0.5 flex items-center gap-1.5">
            ...
            <div className="grid grid-cols-1 gap-2.5 my-3 overflow-y-auto max-h-[220px] pr-1">
              {twin.current_state.map((state, idx) => (
                <div key={idx} className="grid grid-cols-2 gap-3 p-3 rounded-xl bg-zinc-950/40 border border-zinc-900">
                  <div className="flex flex-col gap-1">
                    <span className="text-[9px] font-bold text-zinc-500 uppercase">Situation Actuelle</span>
                    <p className="text-xs text-zinc-400 font-medium leading-normal">{state}</p>
                  </div>
      ...
      case 2:
        return (
          <div className="flex flex-col h-full justify-between animate-fade-in p-5">
            ...
            <div className="my-2 p-3 bg-zinc-950/30 rounded-xl border border-zinc-900 flex flex-col gap-4">
              {metrics.map((m, idx) => (
                <div key={idx} className="flex flex-col gap-1.5">
                  ...
                  <div className="relative h-6 bg-zinc-950 rounded-lg overflow-hidden border border-zinc-900 flex items-center px-1">
                    {/* Before Bar */}
                    <div 
                      className="absolute left-0 top-0 bottom-0 bg-zinc-800/60 border-r border-zinc-700 transition-all duration-1000"
            ...
            <div className="my-2 overflow-y-auto max-h-[220px] flex flex-col gap-2 pr-1">
              {services.map((s, idx) => (
                <div key={idx} className="p-3 rounded-xl bg-zinc-950/40 border border-zinc-900 flex flex-col gap-1">
                  ...
                  <p className="text-[11px] text-zinc-400 font-medium leading-normal">{s.reasoning}</p>
                </div>
              ))}
            </div>
          ...
          <div className="flex flex-col h-full justify-between animate-fade-in p-5">
            ...
            <div className="my-2 overflow-y-auto max-h-[220px] flex flex-col gap-2 pr-1">
              {twin.roadmap.map((step, idx) => (
                <div key={idx} className="flex gap-3 items-start p-2.5 rounded-xl bg-zinc-950/40 border border-zinc-900">
                  ...
                  <div className="flex flex-col">
                    <span className="text-xs font-semibold text-zinc-200">Étape {idx + 1}</span>
                    <p className="text-[11px] text-zinc-400 font-medium leading-relaxed mt-0.5">{step}</p>
                  </div>
                </div>
              ))}
            </div>
  ...
  return (
    <div className="glass-card rounded-2xl border border-zinc-900/60 shadow-lg flex flex-col justify-between w-full h-[400px] overflow-hidden animate-fade-in relative">
      ...
      <div className="p-4 border-t border-zinc-900/60 bg-zinc-950/20 shrink-0 flex items-center justify-between">
        <button
          onClick={handlePrev}
          className="p-1.5 rounded-lg border border-zinc-800 hover:border-zinc-700 bg-zinc-900/60 text-zinc-300 hover:text-zinc-100 transition-all cursor-pointer flex items-center justify-center"
        >
          ...
          {Array.from({ length: totalSlides }).map((_, idx) => (
            <button
              ...
              className={`w-1.5 h-1.5 rounded-full transition-all cursor-pointer ${
                currentSlide === idx ? 'w-4 orange-gradient-bg' : 'bg-zinc-700 hover:bg-zinc-600'
              }`}
