...
`;
  ...
  const totalChurnItems = radarData?.churn_alerts.length || 0;
  const totalChurnPages = Math.max(1, Math.ceil(totalChurnItems / PAGE_SIZE));
  const paginatedChurnAlerts = (radarData?.churn_alerts || []).slice(
    (churnPage - 1) * PAGE_SIZE,
    churnPage * PAGE_SIZE
  );

  const totalUpsellItems = radarData?.upsell_opportunities.length || 0;
  const totalUpsellPages = Math.max(1, Math.ceil(totalUpsellItems / PAGE_SIZE));
  const paginatedUpsellOpportunities = (radarData?.upsell_opportunities || []).slice(
    (upsellPage - 1) * PAGE_SIZE,
    upsellPage * PAGE_SIZE
  );
  ...
  return (
    <div className="flex-1 flex flex-col h-full bg-[#ECEAE5] dark:bg-[#242124] overflow-y-auto p-6 md:p-8 select-none transition-colors duration-300">
      ...
      <div className="mb-6 space-y-4 pb-6 border-b border-black/5 dark:border-white/5">
          ...
          <div>
            <div className="flex items-center gap-2 mb-1">
              ...
              <span className="text-[11px] text-[#6E6C67] dark:text-[#A1A1AA]">
                Détection préventive Taux d&apos;abandon & Opportunités de Vente additionnelle
              </span>
            </div>
        ...
        {/* 2 Tabs: Taux d'abandon vs Vente additionnelle */}
      ...
      {loading ? (
        ...
        <div className="space-y-4">
          {totalChurnItems === 0 ? (
            ...
            <>
              {paginatedChurnAlerts.map((alert) => (
                <div
                  key={alert.id}
              ...
              <Pagination
                currentPage={churnPage}
                totalPages={totalChurnPages}
                totalItems={totalChurnItems}
                pageSize={PAGE_SIZE}
                itemName="comptes à risque"
        ...
        /* VENTE ADDITIONNELLE OPPORTUNITIES LIST */
        <div className="space-y-4">
          {totalUpsellItems === 0 ? (
            <div className="p-12 text-center bg-[#FFFFFF] dark:bg-[#2F2C30] rounded-3xl space-y-2">
            ...
            <>
              {paginatedUpsellOpportunities.map((opp) => (
                <div
                  key={opp.id}
              ...
              <Pagination
                currentPage={upsellPage}
                totalPages={totalUpsellPages}
                totalItems={totalUpsellItems}
                pageSize={PAGE_SIZE}
                itemName="opportunités"
                onPageChange={setUpsellPage}
              />
            </>
          )}
        </div>
