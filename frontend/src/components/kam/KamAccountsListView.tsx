...
export default function KamAccountsListView({
  ...
  // Helper to determine segment (used strictly for filtering criteria)
  ...
  // Filter accounts according to search query & segment
  ...
  const activeFiltersCount = selectedSegment !== 'ALL' ? 1 : 0;

  const handleResetFilters = () => {
    setSelectedSegment('ALL');
  };

  // Sort accounts by priority: recent incidents/churn risk first, then highest MRR
  const sortedAccounts = [...filteredVisits].sort((a, b) => {
  ...
  return (
    <div className="flex-1 flex flex-col gap-6 p-8 overflow-y-auto select-none">
        ...
        <div className="flex flex-wrap items-center justify-between gap-4">
          <div>
            ...
            <p className="text-xs text-zinc-500 dark:text-zinc-400 mt-0.5">
              Suivi opérationnel et accès direct aux fiches 360°.
            </p>
          </div>
          ...
          <div className="flex items-center gap-2.5">
            ...
            >
              ...
              {activeFiltersCount > 0 && (
                <span className="w-5 h-5 rounded-full bg-white text-[#4F6CE8] text-[10px] font-extrabold flex items-center justify-center">
                  {activeFiltersCount}
                </span>
              )}
            </button>
      ...
      {/* 2. UNIFIED ACCOUNTS GRID (NO SECTOR SEGMENTATION) */}
              ...
              return (
                ...
                >
                  <div>
                      ...
                      <div className="flex flex-col items-end gap-1">
                        <span className={`px-2.5 py-0.5 rounded-full text-[10px] font-extrabold flex items-center gap-1.5 ${
                          visit.conversion_status === 'CONVERTED'
                            ...
                            ? 'bg-blue-500/15 text-[#4F6CE8] dark:text-blue-400 border border-blue-500/20'
                          ...
                          <span className={`w-1.5 h-1.5 rounded-full ${
                            ...
                            visit.conversion_status === 'IN_NEGOTIATION' ? 'bg-[#4F6CE8]' :
                    ...
                    {/* Account Name & Info */}
                    <h4 className="text-base font-semibold text-zinc-900 dark:text-white group-hover:text-[#4F6CE8] dark:group-hover:text-blue-400 transition-colors leading-snug">
                      {visit.account_name}
                    </h4>
