...
class PiCamera2Adapter(BaseCamera):
    ...
    def start(self) -> None:
        with self._lock:
            ...
            self.picam2.start()
            self.is_active = True
            time.sleep(0.5)  # Warmup for auto-exposure/white balance
    ...
    def capture_raw_frame(self) -> np.ndarray:
        with self._lock:
            ...
            # Capture array from picam2 (returns RGB)
            rgb_array = self.picam2.capture_array()
            # Convert RGB to BGR for OpenCV processing pipeline consistency
